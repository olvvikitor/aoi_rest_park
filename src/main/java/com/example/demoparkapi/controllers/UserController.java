...
public class UserController {
	...
	@Operation(summary = "Buscar um usuário por ID", description = "Recurso para Buscar um Usuario por ID", responses = {
	...
	@Operation(summary = "Alteração de senha", description = "Recurso para Alteração de senha", responses = {
	...
	public ResponseEntity<String> updatePassword(@PathVariable Long id, @Valid @RequestBody UserPasswordDTO userDto) {
		userService.alterPassword(id, userDto.getSenhaAtual(), userDto.getNovaSenha(), userDto.getConfirmaSenha());
		return ResponseEntity.status(HttpStatus.OK).body("Senha alterada com sucesso");
	}

	@Operation(summary = "Busca por todos os usuarios", description = "Recurso para buscar todos os usuarios", responses = {
			@ApiResponse(responseCode = "200", description = "Busca realizada com sucesso", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserResponseDto.class))) })
	@GetMapping
